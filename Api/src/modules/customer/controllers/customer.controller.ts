...
export class CustomerController {
   ...
   * Add loyalty points
   */
  addLoyaltyPoints = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, _next: NextFunction) => {
      const { points } = req.body;
      const updatedCustomer = await this.customerService.addLoyaltyPoints(
        req.user._id,
        points,
      );

      res.status(200).json(
        Helpers.formatResponse(true, 'Loyalty points added successfully', {
   ...
   */
  redeemLoyaltyPoints = asyncHandler(
    async (req: AuthenticatedRequest, res: Response, _next: NextFunction) => {
      const { points } = req.body;
      const updatedCustomer = await this.customerService.redeemLoyaltyPoints(
        req.user._id,
        points,
      );

      res.status(200).json(
        Helpers.formatResponse(true, 'Loyalty points redeemed successfully', {
          loyaltyPoints: updatedCustomer.loyaltyPoints,
        }),
      );
    },
